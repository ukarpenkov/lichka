...
export const ru: LocaleBundle = {
  ...
  dictionary: {
    ...
    backupTooLarge: 'Бэкап больше 25 МБ. Удалите часть медиа или сохраните копию через «Экспорт в файл».',
    driveAuthDeveloper: 'Google не принял приложение (неверная подпись SHA-1). Для debug-сборки нужен debug-сертификат, для установки из Play — SHA-1 из Play Console → App signing.',
    driveAuthPlayServices: 'Google Play Services недоступны или требуют обновления.',
    driveAuthDenied: 'Нет доступа к данным приложения в Google Drive. Войдите тестовым аккаунтом из списка Test users и разрешите доступ.',
